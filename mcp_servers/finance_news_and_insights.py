...
EXTERNAL_FINANCE_NEWS_API_BASE_URL = os.getenv("EXTERNAL_FINANCE_NEWS_API_BASE_URL", "https://www.alphavantage.co/query") # Example Alpha Vantage URL

if not EXTERNAL_FINANCE_API_KEY:
    raise ValueError("EXTERNAL_FINANCE_API_KEY not found in environment variables. Please get an API key from a financial data provider.")

mcp = FastMCP(name="Finance MCP Server - News and Insights")

@mcp.tool(description="Retrieves recent financial news for a specific company or general market news.")
def get_financial_news(symbol: str = None, limit: int = 5) -> dict:
    ...
    try:
        ...
        if "feed" not in data or not data["feed"]:
            return {"error": f"No news found for {symbol if symbol else 'general market'}."}
        ...
        for article in data["feed"][:limit]:
            news_articles.append({
                ...
                "published_date": article.get("time_published") # Alpha Vantage uses time_published
            })
