...
DB_URI = os.getenv(
    "FINANCE_DB_URI", None
)  # Assuming portfolio data is in the finance DB
...
def add_stock_holding(
    user_id: int, symbol: str, quantity: int, purchase_price: float, purchase_date: str
) -> dict:
    """
    ...
    try:
        ...
        with psycopg2.connect(USER_DB_URI) as conn_user:
            with conn_user.cursor() as cur_user:
                cur_user.execute(
                    'SELECT id FROM public."USERS" WHERE id = %s', (user_id,)
                )
        ...
        with psycopg2.connect(DB_URI) as conn:
            with conn.cursor() as cur:
                ...
                if existing_holding:
                    ...
                    new_total_cost = (old_quantity * old_purchase_price) + (
                    ...
                    return {
                        "success": True,
                        "message": f"Updated holding for {symbol} for user {user_id}. New quantity: {new_total_quantity}, Average price: {new_average_price:.2f}",
                    }
                else:
                    # Insert new holding
                    cur.execute(
                    ...
                    return {
                        "success": True,
                        "message": f"Added {quantity} shares of {symbol} for user {user_id}.",
                    }
    except Error as e:
                    ...
                    holdings.append(
                        {
                            ...
                            "gain_loss_percentage": (
                                (latest_price - Decimal(str(purchase_price)))
                                / Decimal(str(purchase_price))
                            )
                            * Decimal("100.0")
                            if purchase_price != 0
...
@mcp.tool(
    description="Removes a stock holding from a user's portfolio or reduces quantity."
)
def remove_stock_holding(
    user_id: int, symbol: str, quantity_to_remove: int = None
) -> dict:
    """
        ...
        with psycopg2.connect(DB_URI) as conn:
            with conn.cursor() as cur:
                ...
                if not existing_holding:
                    return {
                        "error": f"Holding for {symbol} not found for user {user_id}."
                    }
                ...
                if quantity_to_remove is None or quantity_to_remove >= current_quantity:
                    ...
                    cur.execute(
                        'DELETE FROM public."USER_STOCK_HOLDINGS" WHERE id = %s',
                        (holding_id,),
                    )
                    conn.commit()
                    return {
                        "success": True,
                        "message": f"All shares of {symbol} removed from user {user_id}'s portfolio.",
                    }
                else:
                    # Reduce quantity
                    ...
                    return {
                        "success": True,
                        "message": f"Removed {quantity_to_remove} shares of {symbol}. Remaining: {new_quantity} shares.",
                    }
    except Error as e:
        return {"error": f"Database error: {str(e)}"}
    except Exception as e:
